...
public class TestCases extends ExcelDataProvider { // Lets us read the data
        ...
        public static void testCase01() throws InterruptedException {
                ...
                List<WebElement> aboutText = driver
                                .findElements(By.xpath("//p[contains(@class,'lb-font-color-text-primary')]"));
        ...
        public static void testCase02() throws InterruptedException {
                ...
                driver.get("https://www.youtube.com/");
                Thread.sleep(3000);
                WebElement moviesElement = driver.findElement(By.xpath("//yt-formatted-string[text()='Movies']"));
                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", moviesElement);
                moviesElement.click();
                Thread.sleep(5000);
                ...
                // "Drama"
                softAssert.assertTrue(movieText.contains("Animation") || movieText.contains("Comedy")
        ...
        //                         .findElement(By.xpath("//span[text()='India's Biggest Hits']"));
        //         ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", indiaElement);
                

        //         Wrappers.clickUntilElementVisible(driver, By.xpath(
        //                         "(//*[@id='right-arrow']/ytd-button-renderer/yt-button-shape/button)[3]"),
        //                         3);
...
public static void testCase03() {
        ...
        try {
            ...
            // Locate and click on the "Music" section
            WebElement musicElement = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//yt-formatted-string[text()='Music']")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", musicElement);
            musicElement.click();
    
            // Wait for the page to load
            Thread.sleep(3000); // Optional: Replace with a proper wait if possible
    
            // Locate "India's Biggest Hits" element
            WebElement indiaElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//span[text()=\"India's Biggest Hits\"]")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", indiaElement);
    
            // Click the right arrow button in the carousel
            Wrappers.clickUntilElementVisible(driver, By.xpath(
                "(//*[@id='right-arrow']/ytd-button-renderer/yt-button-shape/button)[3]"), 3);
    
            Thread.sleep(3000); // Optional: Replace with WebDriverWait if content takes time to load
    
            // Validate the header for "India's Biggest Hits"
            WebElement headerElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//span[contains(text(),\"India's Biggest Hits\")]/ancestor::div[@id='dismissible']//h3[normalize-space(text())='Bollywood Dance Hitlist']")));
            String headerText = headerElement.getText();
            System.out.println("Header of music element is: " + headerText);
    
            // Validate track count
            WebElement trackElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//span[contains(text(),\"India's Biggest Hits\")]/ancestor::div[@id='dismissible']//h3[normalize-space(text())='Bollywood Dance Hitlist']/following::p[@id='video-count-text'][1]")));
            String trackText = trackElement.getText();
            trackText = trackText.replaceAll("[^0-9]", "").trim();
